...
module ApplicationHelper
  ...
  def shortcut_menus
    ...
    option_tags = [content_tag(:option, _('Move to %{group} joined ...') % {:group => name_of_group}, :value => url_for({:controller => '/mypage', :action => 'group'}))]
    ...
    if groups = Group.active.participating(current_user).order_participate_recent.all(:include => :group_category) and !groups.empty?
      groups.group_by(&:group_category).sort_by{|c, g| c.sort_order}.each do |category, groups_by_category|
        ...
        groups_by_category.each do |group|
          option_tags << content_tag(:option, "&nbsp;#{truncate(h(group.name), :length => 18)}", :value => url_for({:controller => '/group', :gid => group.gid, :action => 'show', :from => 'shortcut'}))
        end
      end
    end
