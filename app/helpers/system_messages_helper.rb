...
module SystemMessagesHelper
  def system_message_links
    ...
    current_user.system_messages.each do |sm|
      ...
      system_message_links << link_to(icon_tag(system_message_data[:icon]) + system_message_data[:message], system_message_data[:url]) + "(#{link_to 'x', [current_tenant, current_user, sm], :class => 'delete_system_message', :method => 'delete'})" unless system_message_data.blank?
    end
