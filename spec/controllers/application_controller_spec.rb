...
describe ApplicationController, "#sso" do
  describe "固定OP利用設定の場合" do
    before do
      ...
      tenant.initial_settings['fixed_op_url'] = 'http://localhost.com/'
    end
    describe "未ログイン時" do
      before do
      ...
      describe '通常のリクエストの場合' do
        ...
        it "ログインへリダイレクトされる" do
          controller.stub!(:logged_in?).and_return(false)
          controller.should_receive(:redirect_to).with({:controller => '/platform', :action => :login, :openid_url => tenant.initial_settings['fixed_op_url'], :return_to => @return_to})
          controller.send(:sso).should be_false
        end
      end
    end
