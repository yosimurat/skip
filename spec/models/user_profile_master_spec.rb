  ...
  describe UserProfileMaster, 'validation' do
    describe UserProfileMaster, '.validates_presence_of_category' do
      ...
      describe 'user_profile_master_categoryに対象のカテゴリが存在する場合' do
        before do
          @user_profile_master = create_user_profile_master(:tenant => @sg, :user_profile_master_category_id => create_user_profile_master_category(:tenant => @sg).id)
        end
        it { should have(0).errors_on(:user_profile_master_category_id) }
      end
      describe 'user_profile_master_categoryに対象のカテゴリが存在しない場合' do
        before do
          @user_profile_master = create_user_profile_master
        end
        it { should have(1).errors_on(:user_profile_master_category_id) }
      end
    end
  end
  ...
  describe UserProfileMaster::SelectProcesser do
    ...
    describe "#to_edit_html" do
      it "selectタグが含まれるhtmlが返ること" do
        @processer.to_edit_html(@value).should have_selector("select")
      end
      it "データが存在している場合選択されていること" do
        @value.value = 'select2'
        @processer.to_edit_html(@value).should have_selector("select") do |select_tag|
          select_tag.inner_html.should have_selector('option', :selected => 'selected', :value => 'select2')
        end
      end
    end
  end
    ...
    describe "#to_edit_html" do
      describe "既に登録されている値がある場合" do
        ...
        it "selectボックスが表示されていること" do
          @processer.to_edit_html(@value).should have_selector("select") do |select_tag|
            select_tag.inner_html.should have_selector('option', :value => 'value3')
          end
        end
        it "valueがselectボックスの値にある場合 selectボックスが選択されていること" do
          @value.value = "value1"
          @processer.to_edit_html(@value).should have_selector("select") do |select_tag|
            select_tag.inner_html.should have_selector('option', :selected => 'selected', :value => 'value1')
          end
        end
        it "text_fieldが表示されていること" do
          @processer.to_edit_html(@value).should have_selector("input[type=text]")
        end
        it "valueがselectボックスの値に無い場合 text_field内に表示されていること" do
          @value.value = "text_input"
          @processer.to_edit_html(@value).should have_selector("input[value=text_input]")
        end
      end
      describe "登録されている値が無い場合" do
        ...
        it "selectボックスが表示されていないこと" do
          @processer.to_edit_html(@value).should_not have_selector("select")
        end
        it "text_fieldが表示されていること" do
          @processer.to_edit_html(@value).should have_selector("input[type=text]")
        end
    ...
    describe "#to_edit_html" do
      ...
      describe "何も選択されていない場合" do
        it "checkboxのhtmlを返すこと" do
          @value.value = ""
          @processer.to_edit_html(@value).should have_selector("input#profile_value_#{@master.id}_check1")
        end
      end
