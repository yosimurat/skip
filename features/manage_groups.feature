Feature: グループの管理
  ...
  Scenario: 文書機能を作成することができる
    ...
    And "vim_groupグループのトップページ"にアクセスする
    And "管理"リンクをクリックする
    And "この機能を利用しますか？"から"はい"を選択する
    And "更新"ボタンをクリックする
    And "トップ"リンクをクリックする

    Then "目次を作る"と表示されていること

    # グループに参加していないユーザは目次を作成できない
    When "a_user"でログインする
    And "vim_groupグループのトップページ"にアクセスする
    Then "目次を作る"と表示されていないこと

    # 目次を作成することができる
    When "alice"でログインする
    And "vim_groupグループのトップページ"にアクセスする
    And "目次を作る"リンクをクリックする
    And "タイトル"に"目次タイトル"と入力する
    And "作成"ボタンをクリックする

    Then "目次タイトル"と表示されていること
    And "「目次タイトル」配下に文書を作る"と表示されていること

    # トップ文書配下に子文書が作成できる
    When "「目次タイトル」配下に文書を作る"リンクをクリックする
    And "タイトル"に"子文書タイトル"と入力する
    And "Wikiテキスト"を選択する
    And "内容"に"内容"と入力する
    And "作成"ボタンをクリックする

    Then "目次タイトル"と表示されていること
    And "子文書タイトル"と表示されていること
    And "「子文書タイトル」配下に文書を作る"と表示されていること

    When "vim_groupグループのトップページ"にアクセスする
    Then "子文書タイトル"と表示されていること
